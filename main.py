...
from utils.tools import draw_info, result_info_format, format_data, writexls, writecsv, resize_with_padding, \
    open_image_with_default_viewer
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, cfg=None):
        ...
        # 保存csv内容
        self.result_csv = os.path.join(result_time_path, 'result.csv')
        # 写入CSV文件
        with open(self.result_csv, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            # 写入表头
            writer.writerow(['序号', '图片路径', '录入时间', '识别结果', '目标数目', '识别速度', '保存路径'])
        ...
        self.label_img_path.installEventFilter(self)
        self.label_dir_path.installEventFilter(self)
        self.label_video_path.installEventFilter(self)
        self.label_camera_path.installEventFilter(self)
        ...
        # 双击事件绑定
        self.tableWidget_info.cellDoubleClicked.connect(self.cell_double_clicked)
        self.tableWidget_info.setToolTip("双击可查看识别结果图片")
    ...
    def init_UI_config(self):
        ...
        # 更新title字体颜色
        self.label_title.setStyleSheet("color: rgb(%s);" % title_color)
        # 更新背景图片
        ...
        # 是否显示摄像头功能
        if not show_camera:
            self.label_camera_path.hide()
            self.pushButton_camera.hide()

    def eventFilter(self, obj, event):
        # 点击标签时也执行对应操作
        if event.type() == QEvent.MouseButtonPress:
            if obj == self.label_img_path:
                self.open_img()
                return True
            if obj == self.label_dir_path:
                self.open_dir()
                return True
            if obj == self.label_video_path:
                self.open_video()
                return True
            if obj == self.label_camera_path:
                self.open_camera()
                return True
    ...
    def cell_clicked(self, row, column):
        ...
        # 读取序号
        number = self.tableWidget_info.item(row, 0).text()
        # 图片路径
        self.img_path = self.tableWidget_info.item(row, 1).text()
        # 读取csv
        with open(self.result_csv, 'r', encoding='utf-8') as csvfile:
    ...
    def cell_double_clicked(self, row, column):
        ...
        """
        try:
            # 判断是不是第6列（第7列下标是6，注意从0开始）
            # if column == 6:
            #     # 判断单元格是否有内容
            #     if self.tableWidget_info.item(row, column) is None:
            #         return

            # 获取图片路径
            # img_path = self.tableWidget_info.item(row, column).text()
            img_path = self.tableWidget_info.item(row, 6).text()
            if not os.path.exists(img_path):
                print(f"图片路径不存在: {img_path}")
                return
            # 使用本地的图片工具打开图片
            open_image_with_default_viewer(img_path)  # 调用打开函数
    ...
    def onComboBoxActivated(self):
        ...
        if len(self.results) == 0:
            QMessageBox.information(self, "信息", "目标无效。", QMessageBox.Yes)
            return
        # 所有目标，默认显示结果中的第一个
        ...
        if self.img_path == 'camera':
            QMessageBox.information(self, "信息", '此功能仅支持图片识别及遍历文件夹识别！', QMessageBox.Yes)
            return 0
        else:
            self.img = cv2.imdecode(np.fromfile(self.img_path, dtype=np.uint8), cv2.IMREAD_COLOR)
            if self.img is not None:
                self.img_show = draw_info(self.img, lst_info, draw_chinese, chinese_name)
                self.show_all(self.img_show, result_info)
            else:
                QMessageBox.information(self, "信息", '此功能仅支持图片识别及遍历文件夹识别！', QMessageBox.Yes)
    ...
    def open_img(self):
        try:
            ...
            self.img_path, filetype = QFileDialog.getOpenFileName(None, "选择文件", self.ProjectPath,
                                                                  "Image Files (*.jpg *.jpeg *.png *.jfif *.bmp *.webp)")
    ...
    def start(self):
        ...
        try:
            if self.start_type == None:
                QMessageBox.information(self, "信息", "选择要识别的图片或视频！", QMessageBox.Yes)
                return
            ...
            if self.frame_number == 0 and self.start_type == 'video':
                ...
                if not ret:
                    ...
                    if self.start_type == 'video':
                        ...
                        self.label_camera_path.setText(" 摄像头已关闭")
                        QMessageBox.information(self, "信息", "摄像头关闭，请点击摄像头图标，打开摄像头。", QMessageBox.Yes)
                    return
                if self.start_type == 'video':
                    # 获取当前帧号并设置图像名称
                    frame_number = int(self.video.get(cv2.CAP_PROP_POS_FRAMES))
    ...
    def predict_img(self, img):
        ...
        # 模型识别
        self.results = yolo.predict(img, imgsz=imgsz, conf=conf_thres, device=device)
        # 计算并记录消耗时间
        self.consum_time = str(round(time.time() - t1, 2)) + 's'
        # self.consum_time = str(int(1 / round(time.time() - t1, 2))) + ' fps'
        # 整理格式
        self.results, self.tab_res_list = format_data(self.results, self.chinese_name, draw_chinese)
        # 记录输入时间
        self.input_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # 将结果写入文件
        # 写入CSV文件
        with open(self.result_csv, 'a+', newline='', encoding='utf-8-sig') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(
        ...
        result_info = result_info_format(result_info, box, score, cls_name)
        # 在图像上绘制识别结果
        self.img_show = draw_info(img, self.results, draw_chinese, chinese_name)
        # 保存结果图像
    ...
    def show_info(self, result):
        try:
            ...
            # 如果启用了中文标签，则使用中文标签
            if draw_chinese and cls_name in chinese_name:
                cls_name = self.chinese_name[cls_name]

            self.label_class.setText(str(cls_name))

            # 自动缩放文本以适应标签
            font = self.label_class.font()
            fm = QFontMetrics(font)
            elided_text = fm.elidedText(cls_name, QtCore.Qt.ElideRight, self.label_class.width())
            self.label_class.setText(elided_text)
    ...
    def show_table(self):
        try:
            ...
            tab_res_list = ','.join(str(x) for x in self.tab_res_list)
            for column, content in enumerate(
                    [self.number, self.img_path, self.input_time, tab_res_list, len(self.results), self.consum_time,
                     self.result_img_name]):
    ...
    def write_files(self):
        """
        导出 self.tableWidget_info 表格中的数据为 Excel 或 CSV
        """
        ...
        # 获取表格数据
        row_count = self.tableWidget_info.rowCount()
        col_count = self.tableWidget_info.columnCount()
        data = []

        # 获取表头
        headers = []
        for col in range(col_count):
            item = self.tableWidget_info.horizontalHeaderItem(col)
            headers.append(item.text() if item else f"列{col + 1}")
        data.append(headers)
...
if __name__ == "__main__":
    ...
    title = cfg_UI.title
    title_color = cfg_UI.label_title_color
    ...
    draw_chinese = cfg.CONFIG.draw_chinese
    show_camera = cfg.CONFIG.show_camera
